...
VertexOut VS(VertexIn vin, uint instanceID : SV_InstanceID)
{
  ...
  // test.y = test.y+instanceID;
   //vin.PosL.y = vin.PosL.y+instanceID;
    ...
    float4 posW = mul(float4(test, 1.0f), res_world);
    vout.PosW = posW.xyz;
...
float4 PS(VertexOut pin) : SV_Target
{
    ...
    float heightFog =  HeightFog(pin.PosW,gEyePosW,1.0,gFogColor.xyz,gFogStart,gFogRange);
    float fogAmount = saturate(heightFog*((distToEye - 2) /100));
    //fogAmount =max(fogAmount, saturate(HeightFog(pin.PosW.xyz,gEyePosW.xyz,1.0,gFogColor.xyz)) );
